...
def process_groups(score_array, match_array)
  ...
  while cur_chunk_ind < len
    if cur_chunk_ind + chunk_size > len
      ...
      tmp_score_array = score_array[cur_chunk_ind, chunk_size]
      tmp_match_array = match_array[cur_chunk_ind, chunk_size]
    end

    count = tmp_match_array.count_true
    perc = ("%5.4f" % (count.to_f / tmp_match_array.size.to_f * 100)).to_f
    tmp_array = Array.new
...
Benchmark.bm do |make|
  ...
  make.report do
    Dir.glob( File.join(dir, '*.dat') ) do |file|
      filename = file.to_s[file.rindex('/')+1..-1]
      piece_size = filename.split('-')[1]
      date = filename.split('-')[2].chomp('.dat')
      ...
      score_vals.each { |key,value|
        ...
        puts "processing: #{key}-mers | num elements: #{value.length}"
        ...
        low = ("%5.5f" % score_arr[0]).to_f if key == '3'
        high = ("%5.5f" % score_arr[score_arr.size - 2]).to_f if key == '8'
      ...
      low /= piece_size.to_f; high /= piece_size.to_f
      incr = ("%5.3f" % ((high - low) / 5.0)).to_f
      puts "low: #{low} || high: #{high} || incr: #{incr}"
      ...
      graph_x_vals.each { |val|
        graph.labels[val] = ("%3.4f" % (cur_score_val + incr))
        cur_score_val += incr
      }
